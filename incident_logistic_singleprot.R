...
library(broom)
library(survival)
library(PheWAS, lib.loc = "/users/danifusc/R_library")
library(ggplot2)
library(pheatmap)
library(patchwork)
...
allprs= fread("data/idefix_olinksoma/geno_residuals.tsv")
### scale PRS
...
write.table(incident_prot, file="data/incident_logreg_singleproteins.tsv", sep="\t", quote=F,
...
write.table(incident_prs, file="data/incident_logreg_sSinglePRS.tsv", sep="\t", quote=F,
            row.names = F, col.names = T)


incident_residnocov= incident_logif(resid_nocov, "IndID")
incident_residnocov = incident_residnocov %>% rename(estimate_resid= Estimate, SE_resid= `Std. Error`,
                                           zvalue_resid= `z value`, pval_resid =`Pr(>|z|)`)
incident_residnocov$protein= gsub("StandResid_","", incident_residnocov$protein) 
write.table(incident_residnocov, file="data/incident_logreg_singleresidualsnocov.tsv", sep="\t", quote=F,
            row.names = F, col.names = T)
